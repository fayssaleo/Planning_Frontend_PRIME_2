<template>
  ...
  <v-row style="width: 100%; margin-bottom: 51px" class="mt-0">
    <v-data-table :headers="headers" :items="thisPlanningBoxes" class="elevation-1 tableOfThePlanning"
      hide-default-footer :items-per-page="thisPlanningBoxes.length" style="width: 90%; margin: 0 auto">
      <template v-slot:thead>
        <thead>
          <tr v-if="editMode" class="addDriverButtonClickableCase">
            ...
            <th></th>
            <th v-for="header in headers.slice(2)" :key="header.key" style="position: relative; margin: 0; padding: 0">
    ...
    <div class="pa-4 text-center">
      <v-dialog max-width="100%" v-model="saveDialogShow">
        <template v-slot:default="{ isActive }">
          <v-card :title="'RTG SHIFT PLANNING - ' +
            actualShift.name +
            ' (Date: ' +
            selectedDate.toLocaleDateString() +
            ' - ' +
            ...
            <v-card-actions class="mt-5">
              ...
              <v-btn density="default" class="saveButton2" @click="openSaveCONFIRMDialog">SAVE</v-btn>
            </v-card-actions>
          </v-card>
        </template>
      </v-dialog>
        ...
        <v-card class="confirmDialogInterface driversInPlanningWithCheckCard">
          <v-row class="confirmDialogInterfaceHeader">
            <v-col cols="6" class="children" :class="selectedChangeEDSow == 0 ? 'selectedChangeEDSow' : ''"
              @click="selectedChangeEDSow = 0"><v-icon class="thisIcon">mdi-account-group</v-icon>
              DRIVERS</v-col>
            <v-col cols="6" class="children" :class="selectedChangeEDSow == 1 ? 'selectedChangeEDSow' : ''"
              @click="selectedChangeEDSow = 1"><v-icon class="thisIcon">mdi-gantry-crane</v-icon>
              EQUIPMENT</v-col>
          </v-row>
            ...
            <v-col cols="11" :class="allBothAddButton_switch ? 'speadForEdit' : ''" style="margin: 0 auto">
              <v-row>
                ...
                <v-col cols="2" style="margin-bottom: 12px; padding: 0; min-height: 31px"
                  @click="clear" v-if="allBothAddButton_switch">
                ...
                <v-col cols="3" style="margin-bottom: 12px; padding: 0; min-height: 31px" @click="
                  ...
                  <div class="switchEquipment" :class="equipmentEditCheckedForSwitch &&
                      equipmentEditCheckedForSwitchWith || itsAdd
                      ? 'selectebale'
                      : ''
                    " v-if="!allBothNextDrivers && !allBothAddButton_delete">
                    {{itsAdd?'ADD':'SWITCH'}}
                    <span v-if="itsAdd">
                      ...
                      version="1.1" id="Capa_1" viewBox="0 0 309.059 309.059" xml:space="preserve" style="    height: 25px;    width: 50px;">
                              <g>
                              	<g>
                ...
                <v-col cols="11" class="pa-0" style="margin: 0 auto">
                  <v-row style="padding: 0 !important" class="equipmentInRow">
                    <v-col cols="12" :style="{
                      ...
                      <span v-if="allBothAddButton_switch && equipmentEditActions.length>0" class="equipmentActionController"  >
                        <span class="undo" @click="canDoAction?equipmentUndo():''"
                        :class="actionIndex==-1?'whenZero':''"
                        >

                          UNDO
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex]?.action=='add'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="800px" width="800px" version="1.1" id="Capa_1" viewBox="0 0 309.059 309.059" xml:space="preserve" style="height: 18px;width: 18px; margin-top: 1px !important"><g data-v-02c85341=""><g data-v-02c85341=""><path data-v-02c85341="" d="M280.71,126.181h-97.822V28.338C182.889,12.711,170.172,0,154.529,0S126.17,12.711,126.17,28.338    v97.843H28.359C12.722,126.181,0,138.903,0,154.529c0,15.621,12.717,28.338,28.359,28.338h97.811v97.843    c0,15.632,12.711,28.348,28.359,28.348c15.643,0,28.359-12.717,28.359-28.348v-97.843h97.822    c15.632,0,28.348-12.717,28.348-28.338C309.059,138.903,296.342,126.181,280.71,126.181z" style="fill: rgb(33, 150, 243);"></path></g></g></svg>
                          </span>
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex]?.action=='switchV'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="984.85px" height="984.85px" viewBox="0 0 984.85 984.85" xml:space="preserve" style="
                                height: 19px;
                                width: 19px;
                            "><g data-v-02c85341=""><path data-v-02c85341="" d="M652.075,954.791c12.101,12,28.7,20.1,45.7,21.5c17.2,1.399,34.9-2.8,49.2-12.5c3.9-2.601,7.6-5.7,10.9-9l0.1-0.101   l204.9-204.899c29.3-29.3,29.3-76.8,0-106.101c-29.301-29.3-76.801-29.3-106.101,0l-76.899,76.9v-637c0-41.4-33.601-75-75-75   c-41.4,0-75,33.6-75,75v637l-78.9-78.9c-29.3-29.3-76.8-29.3-106.1,0c-29.301,29.301-29.301,76.801,0,106.101l207,207   C651.975,954.69,651.975,954.791,652.075,954.791z" style="fill: rgba(0, 166, 251, 0.77);"></path><path data-v-02c85341="" d="M74.975,365.491c19.2,0,38.4-7.3,53-22l78.9-78.9v637c0,41.4,33.6,75,75,75s75-33.6,75-75v-637l76.899,76.9   c14.601,14.6,33.8,22,53,22s38.4-7.3,53-22c29.3-29.3,29.3-76.8,0-106.1c-5.399-5.4-10.899-10.9-16.3-16.3   c-13.3-13.3-26.5-26.7-39.9-39.9c-17.199-17.1-34.3-34.2-51.399-51.4c-16.8-17-34.2-33.5-50.9-50.7   c-16.2-16.6-32.6-32.9-49.899-48.5c-18-16.2-41.5-24.2-65.7-18.5c-6.2,1.4-12.8,3.8-18.4,6.9c-16.7,9.4-30,24.9-43.5,38.3   c-16.5,16.3-32.7,32.9-49.1,49.4c-19.5,19.6-39.2,39.1-58.8,58.6c-16.4,16.6-33,33.2-49.5,49.8c-8,8-16,15.9-24,23.9   c-0.1,0.1-0.3,0.3-0.4,0.4c-29.3,29.3-29.3,76.8,0,106.1C36.575,358.191,55.775,365.491,74.975,365.491z" style="fill: rgb(0, 40, 85);"></path></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g></svg>                          </span>
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex]?.action=='switchH'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="984.85px" height="984.85px" viewBox="0 0 984.85 984.85" xml:space="preserve" style="height: 18px;width: 18px;transform: rotateZ(90deg);"><g data-v-02c85341=""><path data-v-02c85341="" d="M652.075,954.791c12.101,12,28.7,20.1,45.7,21.5c17.2,1.399,34.9-2.8,49.2-12.5c3.9-2.601,7.6-5.7,10.9-9l0.1-0.101   l204.9-204.899c29.3-29.3,29.3-76.8,0-106.101c-29.301-29.3-76.801-29.3-106.101,0l-76.899,76.9v-637c0-41.4-33.601-75-75-75   c-41.4,0-75,33.6-75,75v637l-78.9-78.9c-29.3-29.3-76.8-29.3-106.1,0c-29.301,29.301-29.301,76.801,0,106.101l207,207   C651.975,954.69,651.975,954.791,652.075,954.791z" style="fill: rgba(0, 166, 251, 0.77);"></path><path data-v-02c85341="" d="M74.975,365.491c19.2,0,38.4-7.3,53-22l78.9-78.9v637c0,41.4,33.6,75,75,75s75-33.6,75-75v-637l76.899,76.9   c14.601,14.6,33.8,22,53,22s38.4-7.3,53-22c29.3-29.3,29.3-76.8,0-106.1c-5.399-5.4-10.899-10.9-16.3-16.3   c-13.3-13.3-26.5-26.7-39.9-39.9c-17.199-17.1-34.3-34.2-51.399-51.4c-16.8-17-34.2-33.5-50.9-50.7   c-16.2-16.6-32.6-32.9-49.899-48.5c-18-16.2-41.5-24.2-65.7-18.5c-6.2,1.4-12.8,3.8-18.4,6.9c-16.7,9.4-30,24.9-43.5,38.3   c-16.5,16.3-32.7,32.9-49.1,49.4c-19.5,19.6-39.2,39.1-58.8,58.6c-16.4,16.6-33,33.2-49.5,49.8c-8,8-16,15.9-24,23.9   c-0.1,0.1-0.3,0.3-0.4,0.4c-29.3,29.3-29.3,76.8,0,106.1C36.575,358.191,55.775,365.491,74.975,365.491z" style="fill: rgb(0, 40, 85);"></path></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g></svg>                          </span>
                        </span>
                        <span class="val">{{(actionIndex+1) }}</span>
                        <span class="do" @click="canDoAction?equipmentRedo():''"
                        :class="actionIndex==equipmentEditActions.length-1?'whenZero':''"
                        >

                          REDO
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex+1]?.action=='add'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="800px" width="800px" version="1.1" id="Capa_1" viewBox="0 0 309.059 309.059" xml:space="preserve" style="height: 15px;width: 15px;margin-top: 1px !important"><g data-v-02c85341=""><g data-v-02c85341=""><path data-v-02c85341="" d="M280.71,126.181h-97.822V28.338C182.889,12.711,170.172,0,154.529,0S126.17,12.711,126.17,28.338    v97.843H28.359C12.722,126.181,0,138.903,0,154.529c0,15.621,12.717,28.338,28.359,28.338h97.811v97.843    c0,15.632,12.711,28.348,28.359,28.348c15.643,0,28.359-12.717,28.359-28.348v-97.843h97.822    c15.632,0,28.348-12.717,28.348-28.338C309.059,138.903,296.342,126.181,280.71,126.181z" style="fill: rgb(33, 150, 243);"></path></g></g></svg>
                          </span>
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex+1]?.action=='switchV'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="984.85px" height="984.85px" viewBox="0 0 984.85 984.85" xml:space="preserve" style="
                                height: 19px;
                                width: 19px;
                            "><g data-v-02c85341=""><path data-v-02c85341="" d="M652.075,954.791c12.101,12,28.7,20.1,45.7,21.5c17.2,1.399,34.9-2.8,49.2-12.5c3.9-2.601,7.6-5.7,10.9-9l0.1-0.101   l204.9-204.899c29.3-29.3,29.3-76.8,0-106.101c-29.301-29.3-76.801-29.3-106.101,0l-76.899,76.9v-637c0-41.4-33.601-75-75-75   c-41.4,0-75,33.6-75,75v637l-78.9-78.9c-29.3-29.3-76.8-29.3-106.1,0c-29.301,29.301-29.301,76.801,0,106.101l207,207   C651.975,954.69,651.975,954.791,652.075,954.791z" style="fill: rgba(0, 166, 251, 0.77);"></path><path data-v-02c85341="" d="M74.975,365.491c19.2,0,38.4-7.3,53-22l78.9-78.9v637c0,41.4,33.6,75,75,75s75-33.6,75-75v-637l76.899,76.9   c14.601,14.6,33.8,22,53,22s38.4-7.3,53-22c29.3-29.3,29.3-76.8,0-106.1c-5.399-5.4-10.899-10.9-16.3-16.3   c-13.3-13.3-26.5-26.7-39.9-39.9c-17.199-17.1-34.3-34.2-51.399-51.4c-16.8-17-34.2-33.5-50.9-50.7   c-16.2-16.6-32.6-32.9-49.899-48.5c-18-16.2-41.5-24.2-65.7-18.5c-6.2,1.4-12.8,3.8-18.4,6.9c-16.7,9.4-30,24.9-43.5,38.3   c-16.5,16.3-32.7,32.9-49.1,49.4c-19.5,19.6-39.2,39.1-58.8,58.6c-16.4,16.6-33,33.2-49.5,49.8c-8,8-16,15.9-24,23.9   c-0.1,0.1-0.3,0.3-0.4,0.4c-29.3,29.3-29.3,76.8,0,106.1C36.575,358.191,55.775,365.491,74.975,365.491z" style="fill: rgb(0, 40, 85);"></path></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g></svg>                          </span>
                          <span class="undoLogo" v-if="equipmentEditActions[actionIndex+1]?.action=='switchH'">
                            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" id="Capa_1" x="0px" y="0px" width="984.85px" height="984.85px" viewBox="0 0 984.85 984.85" xml:space="preserve" style="height: 18px;width: 18px;transform: rotateZ(90deg);"><g data-v-02c85341=""><path data-v-02c85341="" d="M652.075,954.791c12.101,12,28.7,20.1,45.7,21.5c17.2,1.399,34.9-2.8,49.2-12.5c3.9-2.601,7.6-5.7,10.9-9l0.1-0.101   l204.9-204.899c29.3-29.3,29.3-76.8,0-106.101c-29.301-29.3-76.801-29.3-106.101,0l-76.899,76.9v-637c0-41.4-33.601-75-75-75   c-41.4,0-75,33.6-75,75v637l-78.9-78.9c-29.3-29.3-76.8-29.3-106.1,0c-29.301,29.301-29.301,76.801,0,106.101l207,207   C651.975,954.69,651.975,954.791,652.075,954.791z" style="fill: rgba(0, 166, 251, 0.77);"></path><path data-v-02c85341="" d="M74.975,365.491c19.2,0,38.4-7.3,53-22l78.9-78.9v637c0,41.4,33.6,75,75,75s75-33.6,75-75v-637l76.899,76.9   c14.601,14.6,33.8,22,53,22s38.4-7.3,53-22c29.3-29.3,29.3-76.8,0-106.1c-5.399-5.4-10.899-10.9-16.3-16.3   c-13.3-13.3-26.5-26.7-39.9-39.9c-17.199-17.1-34.3-34.2-51.399-51.4c-16.8-17-34.2-33.5-50.9-50.7   c-16.2-16.6-32.6-32.9-49.899-48.5c-18-16.2-41.5-24.2-65.7-18.5c-6.2,1.4-12.8,3.8-18.4,6.9c-16.7,9.4-30,24.9-43.5,38.3   c-16.5,16.3-32.7,32.9-49.1,49.4c-19.5,19.6-39.2,39.1-58.8,58.6c-16.4,16.6-33,33.2-49.5,49.8c-8,8-16,15.9-24,23.9   c-0.1,0.1-0.3,0.3-0.4,0.4c-29.3,29.3-29.3,76.8,0,106.1C36.575,358.191,55.775,365.491,74.975,365.491z" style="fill: rgb(0, 40, 85);"></path></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g><g data-v-02c85341=""></g></svg>                          </span>
                        </span>                      </span>
                      <v-row class="custom-scrollbar equipmentEdit" style="
                        ...
                        " :class="allBothAddButton_switch ? 'toAddOrSwitchEqp' : ''
                          ">
                            ...
                            ">{{
                              equipmentEditCheckedForSwitch &&
                                ...
                                ? checkIfEquipmentIsGoingToBrDeleted
                                  ...
                                  : equipmentEditCheckedForSwitchWith?.matricule
                                : itsAdd?equipmentEditCheckedForSwitch?.matricule:''
                            }}</span>
                        </v-col>
                        <v-col style="text-align: center !important" cols="2" class="ma-0 pa-0">
                          <span v-if="itsAdd">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="800px" width="800px" version="1.1" id="Capa_1" viewBox="0 0 309.059 309.059" xml:space="preserve" style=" height: 50px;    width: 50px;">
                              <g>
                              	<g>
                              		<path style="fill: #2196F3;" d="M280.71,126.181h-97.822V28.338C182.889,12.711,170.172,0,154.529,0S126.17,12.711,126.17,28.338    v97.843H28.359C12.722,126.181,0,138.903,0,154.529c0,15.621,12.717,28.338,28.359,28.338h97.811v97.843    c0,15.632,12.711,28.348,28.359,28.348c15.643,0,28.359-12.717,28.359-28.348v-97.843h97.822    c15.632,0,28.348-12.717,28.348-28.338C309.059,138.903,296.342,126.181,280.71,126.181z"/>
                              	</g>
          ...
          <template v-slot:actions style="width: 100%; /* padding: 0; */ margin-top: 34px">
            <v-spacer></v-spacer>
            <v-btn density="default" class="backButton_all"  :disabled="showSelect" v-if="!allBothAddButton_switch && !allBothAddButton_delete  "
              @click="cancelDriversInPlanningWithCheck">CANCEL</v-btn>
            <v-btn density="default" class="nextButton_all" @click="true" v-if="
              !allBothNextDrivers &&
              !allBothAddButton_delete &&
              !allBothAddButton_switch
            ">FINISH</v-btn>
            ...
            <v-btn density="default" class="nextButton_all" style="background-color: rgb(71 149 52)"
              ...
              <v-btn density="default" class="nextButton_all" style="background-color: rgb(220 47 2 / 74%)"
              @click="openDeleteDialog" v-if="selectedChangeEDSow == 1 && allBothAddButton_delete">DELETE</v-btn>
              <v-btn density="default" class="backButton_all"  v-if="selectedChangeEDSow == 1 && allBothAddButton_switch" :disabled="equipmentEditCheckedForSwitch && equipmentEditCheckedForSwitchWith"
              @click="allBothAddButton_switch=false">BACK</v-btn>
            <v-btn density="default" class="nextButton_all" style="background-color: rgb(71 149 52)"
              @click="openAddDriversForSwitch" v-if="selectedChangeEDSow == 1 && allBothAddButton_switch">CONFIRM</v-btn>
          </template>
        </v-card>
      </v-dialog>
    </div>
...
export default {
  ...
  methods: {
    ...
    equipmentUndo(){
      if(this.canDoAction && this.actionIndex>-1){
        ...
        if(this.equipmentEditActions[this.actionIndex].action=='switchH'){
          this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex].equipmentEditCheckedForSwitchWith;
        this.equipmentEditCheckedForSwitchWith=this.equipmentEditActions[this.actionIndex].equipmentEditCheckedForSwitch;
        ...
        else if(this.equipmentEditActions[this.actionIndex].action=='switchV'){
          this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex].equipmentEditCheckedForSwitch;
        this.equipmentEditCheckedForSwitchWith=this.equipmentEditActions[this.actionIndex].equipmentEditCheckedForSwitchWith;
          console.log("hna 1 this.equipmentEditCheckedForSwitch ",this.equipmentEditCheckedForSwitch )
          console.log("hna 1 this.equipmentEditCheckedForSwitchWith ",this.equipmentEditCheckedForSwitchWith )
          if(this.equipmentsInPlanningWithCheck.filter((e)=>e.id==this.equipmentEditCheckedForSwitch.id).length>0){
            console.log("hna 2 ",)
            this.equipmentsInPlanningWithCheck =this.equipmentsInPlanningWithCheck.map((e) => {
                if (e.id == this.equipmentEditCheckedForSwitch?.id) {
                  console.log("hna 3 ",this.equipmentEditCheckedForSwitch)
                  e = this.equipmentEditCheckedForSwitchWith;
                  return e;
                }
                return e;
                ...
                this.equipmentsNotInPlanningWithCheck.map((e) => {
                  if (e.id == this.equipmentEditCheckedForSwitchWith?.id) {
                    console.log("hna 4 ",this.equipmentEditCheckedForSwitchWith)
                    e = this.equipmentEditCheckedForSwitch;
                    return e;
                  }
                  return e;
            ...
            console.log("hna 5 with",this.equipmentEditCheckedForSwitchWith)
            console.log("hna 5 switch",this.equipmentEditCheckedForSwitch)
            this.equipmentsInPlanningWithCheck=this.equipmentsInPlanningWithCheck.map((e) => {
                if (e.id == this.equipmentEditCheckedForSwitchWith?.id) {
                  console.log("hna 6 with",this.equipmentEditCheckedForSwitchWith)
                  console.log("hna 6 switch",this.equipmentEditCheckedForSwitch)
                  e = this.equipmentEditCheckedForSwitch;
                  return e;
                }
                return e;
                ...
                this.equipmentsNotInPlanningWithCheck.map((e) => {
                  if (e.id == this.equipmentEditCheckedForSwitch?.id) {
                    console.log("hna 7 with",this.equipmentEditCheckedForSwitchWith)
                    console.log("hna 7 switch",this.equipmentEditCheckedForSwitch)
                    e = this.equipmentEditCheckedForSwitchWith;
                    return e;
                  }
                  return e;
        ...
        else if(this.equipmentEditActions[this.actionIndex].action=='add'){
            this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex].equipmentEditCheckedForSwitch;
            let index=this.equipmentEditActions[this.actionIndex].index;
            this.equipmentsNotInPlanningWithCheck.splice(index, 0, this.equipmentEditCheckedForSwitch);
            this.equipmentsInPlanningWithCheck=this.equipmentsInPlanningWithCheck.filter((e)=>{
              return e.id!=this.equipmentEditCheckedForSwitch.id;
            })
            setTimeout(() => {
          this.equipmentEditCheckedForSwitch=null;
    ...
    equipmentRedo(){
      if(this.canDoAction && this.equipmentEditActions.length>this.actionIndex+1){
        this.canDoAction=false;
        if(this.equipmentEditActions[this.actionIndex+1].action=='switchH'){
          this.equipmentEditCheckedForSwitchWith=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitchWith;
        this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitch;
                this.equipmentsInPlanningWithCheck =
                  this.equipmentsInPlanningWithCheck.map((e) => {
                    if (e.id == this.equipmentEditCheckedForSwitchWith?.id) {
                      e = this.equipmentEditCheckedForSwitch;
        ...
        else if(this.equipmentEditActions[this.actionIndex+1].action=='switchV'){
          this.equipmentEditCheckedForSwitchWith=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitchWith;
        this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitch;
        if(this.equipmentsInPlanningWithCheck.filter((e)=>e.id==this.equipmentEditCheckedForSwitch.id).length>0){
            console.log("hna 2 ",)
        ...
        else if(this.equipmentEditActions[this.actionIndex+1].action=='add'){
            this.equipmentEditCheckedForSwitch=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitch;
            this.equipmentEditCheckedForAdd=this.equipmentEditActions[this.actionIndex+1].equipmentEditCheckedForSwitch;
            let index=this.equipmentEditActions[this.actionIndex+1].index;
            this.equipmentsInPlanningWithCheck.push(this.equipmentEditCheckedForSwitch);
            this.equipmentsNotInPlanningWithCheck=this.equipmentsNotInPlanningWithCheck.filter((e)=>{
              return e.id!=this.equipmentEditCheckedForSwitch.id;
            })
            setTimeout(() => {
          this.equipmentEditCheckedForSwitch=null;
    ...
    addEquipment(){
      ...
      let indexTemp=this.equipmentsNotInPlanningWithCheck.indexOf(this.equipmentEditCheckedForSwitch)
      this.equipmentsNotInPlanningWithCheck =
      this.equipmentsNotInPlanningWithCheck.filter((e) => e.id!=this.equipmentEditCheckedForAdd.id);
      this.equipmentEditAddOn= false;
      this.equipmentEditCheckedForAdd= {
        id: null,
        matricule: null,
      ...
      this.actionIndex++;
      this.equipmentEditActions=this.equipmentEditActions.slice(0,this.actionIndex);
    ...
    switchBetweenEquipments() {
      ...
      if (tempOut) {
        thisAction={
          action:"switchV",
          equipmentEditCheckedForSwitch:this.equipmentEditCheckedForSwitch,
          equipmentEditCheckedForSwitchWith:this.equipmentEditCheckedForSwitchWith
        };
        this.equipmentsInPlanningWithCheck =
          this.equipmentsInPlanningWithCheck.map((e) => {
            if (e.id == tempIn?.id) {
      ...
      console.log("this.equipmentEditCheckedForSwitch",this.equipmentEditCheckedForSwitch)
      console.log("this.equipmentEditCheckedForSwitchWith",this.equipmentEditCheckedForSwitchWith)
      this.equipmentEditCheckedForSwitch = null;
      this.equipmentEditCheckedForSwitchWith = null;
      this.actionIndex++;
      this.equipmentEditActions=this.equipmentEditActions.slice(0,this.actionIndex);
