<template>
  <v-row class="saveButtonBar" style="height: 44px !important; color: black" v-if="getJustCreated == false || justEdited==true">
    ...
    <v-col cols="4" style="position: relative">
      ...
      <v-btn density="default" class="saveButton" @click="openSaveDialog">{{ !getJustCreated?'CONFIRM':'UPDATE' }}</v-btn>
    </v-col>
  </v-row>
  <v-row style="width: 90%; margin: 0 auto">
    <v-col class="header" cols="12" style="position: relative">
      ...
      <div class="buttonsHolder2">
        ...
        <div v-if="planningWorkBoxChecked" class="switchBlock" :class="!editMode ? 'displayNone' : ''">
          ...
          <span v-if="planningWorkBoxChecked && planningWorkBoxChecked_With" class="with" @click="doBoxSwitching">
            <svg data-v-02c85341="" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
              ...
              xml:space="preserve" style="height: 30px; width: 30px;     transform: rotateZ(90deg);">
              <g data-v-02c85341="">
        ...
        <div v-if="planningWorkDriver_Checked" class="switchBlock" :class="!editMode ? 'displayNone' : ''">
    ...
    <v-data-table :headers="headers" :items="thisPlanningBoxes"
              ...
              <span class="selectEquipButton fullname" :class="[
                ...
                <span v-if="outs.list.length > 0" class="equipemntOuts">
                  <span v-for="(outItem, index2) in outs.list" :key="index + index2"
                    @click.stop="outItem = clickOnThePlanningWorkBox(outItem, index + 2, rowIndex, true);"
      ...
      <template v-slot:item="{ item, rowIndex }">
        <tr class="rowToEdit" :class="editMode ? 'clickableCase' : ''">
          <td width="15%" @click="item[0] = clickOnThePlanningWorkDriver(item[0], rowIndex, false)" :class="[
          ...
          <td v-for="(itemBox, index) in item.slice(1)" :key="itemBox+index"
            @click="itemBox = clickOnThePlanningWorkBox(itemBox, index + 2, rowIndex, false, (itemBox == 'P' || itemBox == 'DP') ? item[0].id : null)"
            :class="[
            ...
            <div class="sticky-header">
              <v-toolbar flat>
                ...
                <v-btn density="default" class="saveButton2"
                  @click="ifCheckEditedPlanning ? openClickApply() : openSaveCONFIRMDialog()">APPLY</v-btn>
              </v-toolbar>
            </div>
...
export default {
  ...
  methods: {
    ...
    clickOnPauseButton(){
      ...
      else if(this.planningWorkBoxChecked!='P' && this.planningWorkBoxChecked!='DP'){
    ...
    doBoxSwitching() {
      ...
      if (this.workBoxLeftFromOuts) {
        ...
        for (let index = 0; index < this.thisPlanningBoxes.length; index++) {
          const element = this.thisPlanningBoxes[index][this.planningWorkBoxChecked_Index - 1];
          if ((element != 'P' && element != 'DP') && element.matricule == this.planningWorkBoxChecked.matricule) {
